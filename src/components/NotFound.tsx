  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center p-4">
      ...
      >
        {/* 404 Number */}
        <div className="mb-8">
          <div className="text-[150px] md:text-[200px] font-bold leading-none bg-clip-text text-transparent bg-gradient-to-r from-primary via-accent-violet to-primary dark:from-primary-light dark:via-accent-cyan dark:to-primary-light">
            404
          </div>
        </div>
        ...
        <p className="text-lg text-light-text/80 dark:text-dark-text/80 mb-8">
          {language === 'he' 
            ? 'העמוד שחיפשת לא קיים או שהוסר. נסה לרענן את העמוד או לחזור לדף הבית.'
            : "The page you are looking for does not exist or has been removed. Try refreshing or return to homepage."}
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <motion.button
            onClick={handleHome}
            className="inline-flex items-center justify-center px-8 py-4 bg-primary dark:bg-primary-light text-white dark:text-dark-surface rounded-full shadow-lg hover:shadow-xl backdrop-blur-sm"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            <span className="flex items-center justify-center gap-2">
              <Home className="h-5 w-5" />
              <span>{language === 'he' ? 'חזור לדף הבית' : 'Back to Home'}</span>
            </span>
          </motion.button>

          <motion.button
            onClick={handleRefresh}
            className="inline-flex items-center justify-center px-8 py-4 border-2 border-primary dark:border-primary-light text-primary dark:text-primary-light rounded-full hover:bg-primary/5 dark:hover:bg-primary-light/5 transition-colors shadow-lg hover:shadow-xl backdrop-blur-sm"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            <span className="flex items-center justify-center gap-2">
              <RefreshCcw className="h-5 w-5" />
              <span>{language === 'he' ? 'רענן עמוד' : 'Refresh Page'}</span>
            </span>
          </motion.button>
        </div>
      </motion.div>
