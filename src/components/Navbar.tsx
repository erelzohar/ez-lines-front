...
const Navbar: React.FC<NavbarProps> = ({ websiteConfig }) => {
  ...
  return (
    <nav 
      className={`fixed top-4 left-4 right-4 z-50 rounded-2xl mx-auto max-w-7xl transition-all duration-300 transform ${
        isScrolled || isOpen 
    ...
    >
      <div className="px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          >
            <img 
              src={websiteConfig.logoImageName} 
              alt="Logo" 
          ...
          <div className="hidden md:flex items-center space-x-8">
            {menuItems.map((item, index) => (
              <a
                key={index}
                href={item.href}
                className="text-light-text dark:text-dark-text hover:text-primary dark:hover:text-primary-light transition-colors font-medium"
            ...
            {showDarkMode && (
              <button
                ...
                className={`p-2 rounded-full transition-colors ${
                  isScrolled 
                    ? 'bg-light-gray dark:bg-dark-surface hover:bg-primary/10 dark:hover:bg-primary-light/10'
        ...
        >
          {menuItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="block py-3 text-lg text-light-text dark:text-dark-text hover:text-primary dark:hover:text-primary-light transition-colors"
              onClick={handleLinkClick}
            >
              {item.label}
            </a>
